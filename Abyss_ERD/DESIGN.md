...
| 16 | Squad name, acronym, description, level; coach-managed player profiles, game ID/zone, current/highest rank, stars, character pool | `squads`, `squad_coaches`, `player_profiles`, `player_game_profiles`, `squad_memberships`, `player_character_pool`, catalogs |
...
| 18, 26 | Squad statistics, player list, posts, image attachments, optional labels and visibility | `scrim_results`, `squad_memberships`, `squad_posts`, `post_assets` |
...
**User, permanent player identity and game profile are separate.** A representative signs in as a user. A coach can create an unclaimed `player_profiles` row and its `player_game_profiles` row when registering someone who has no ABYSS login. The player later submits `player_profile_claims`; approval links the existing profile to their user account instead of replacing it. One permanent profile can have profiles for multiple games. A squad belongs to one game and mode, and the composite foreign keys prevent cross-game membership, invitations, matches, statistics and ratings.

**Coach, roster and matchmaking are separate from ownership.** `squad_coaches` records the people who represent a squad. One active coach is marked primary; coaches can be granted roster, scrim and opponent-message permissions independently. The create-squad workflow must create its initial primary coach row in the same transaction. Every `squad_memberships` row records the coach who added that player, and every scrim invite records the coach who sent it. Matchmaking reads `vw_matchmaking_squads`, which shows the game, mode, primary coach, all active coaches and the complete current roster. `messages` has a composite foreign key proving that the sender is a coach for the sending squad, while `vw_messages` displays both coaches and both squad names.

**Roster and statistics history are preserved.** End the old `squad_memberships` row by setting `left_at`, then insert a new row during a transfer. Never delete the permanent profile or game profile. `match_participants` stores the membership, squad, game profile and displayed name used at match time; `participant_stat_values` stores metrics through game-specific `stat_definitions`. Past statistics therefore remain attached to the player after a transfer. The baseline permits one active squad per game profile and at most one active captain per squad. `vw_free_agents` shows only claimed, opted-in profiles with no active membership, so coach-created unclaimed people are not publicly listed as independent players.

**Three kinds of rank are different.** `game_ranks` represents a self-reported rank scoped to a supported game; `squad_levels` represents classifications such as Amateur or Collegiate; `squad_ratings` represents an ABYSS competitive score for one game mode and season. `mv_leaderboard` ranks the last of these. OCR does not turn a self-reported rank into an official API-verified value.

**A scrim is a series between exactly two squads in the same game.** `home` and `away` are stable score orientations, not map sides. An invitation becomes at most one scrim. The screenshot shows Best of 5; a series format and game count are stored separately. `fixed_games` additionally supports a drawn series. Player metrics can be entered through `stat_definitions` and `participant_stat_values`; official telemetry remains an optional future source rather than a schema requirement.
...
1. **Initial deployment:** one PostgreSQL instance, application modules, connection pooling, background workers and object storage. Set backup/PITR and restore procedures. Seed supported games, modes, roles, ranks, characters, regions, squad levels, stat definitions and rating rules through migrations. Keep catalog changes audited and refresh affected display objects.
...
The PDF does not settle: exact roster limits, allowed player roles, initial competitive rating/formula, season calendar, minimum leaderboard participation, draw/no-show policy, evidence requirements, visibility options beyond public/squad, record retention, or whether one game profile may join multiple active squads. The SQL makes its proposed assumptions visible and avoids hardcoding a fabricated rank list. Confirm these policies before production implementation; none prevents reviewing or using this schema as the design baseline.
