...
const selectWorkspace = async (id: string, fallbackName: string, fallbackFavorite = false) => {
  ...
  const publishWorkspace = async () => {
    ...
    const rootPublished = await syncWorkspace(selectedWorkspaceId, "PUT", {
      workspace,
      workflows,
      ...rootSnapshot,
    }, true);
    ...
    const workflowResults = await Promise.all(
      workflows
        ...
        .map((workflow) => {
          ...
          return syncWorkspace(remoteWorkspaceId, "PUT", {
    ...
    if (rootPublished && workflowResults.every(Boolean)) {
      setPublishState("success");
      setPublishMessage(`Published ${workflows.length} ${workflows.length === 1 ? "workflow" : "workflows"} successfully.`);
    } else {
      setPublishState("error");
      setPublishMessage("Could not reach the published site. Your local work is still safe.");
    }
  };
  ...
  return (
    <main className="home-shell">
      ...
      <section className="home-main">
        ...
        {isLocalhost && publishMessage ? <div className={`home-publish-message ${publishState}`} role={publishState === "error" ? "alert" : "status"}>{publishMessage}</div> : null}
